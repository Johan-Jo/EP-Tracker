/**
 * Test endpoint that calls the REAL cron job for forgotten checkout alerts
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!cronSecret) {
      console.error('[Test Cron Forgotten Checkout Real] CRON_SECRET not found in environment variables');
      return NextResponse.json(
        { 
    ...
    const cronUrl = `${baseUrl}/api/cron/forgotten-checkout-alerts`;

    console.log('[Test Cron Forgotten Checkout Real] Calling real cron endpoint:', cronUrl);
    ...
    try {
      ...
      console.error('[Test Cron Forgotten Checkout Real] Failed to parse response:', text);
      return NextResponse.json(
        { 
    ...
    console.log('[Test Cron Forgotten Checkout Real] Cron job response:', {
      status: cronResponse.status,
      ok: cronResponse.ok,
      data: cronData,
    });
    ...
    console.error('[Test Cron Forgotten Checkout Real] Unexpected error:', error);
    return NextResponse.json(
