/**
 * Test endpoint for cron check-in reminders
 * Simulates the cron job that sends check-in reminders based on project settings
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Test endpoint should match how cron jobs work in production
    const now = new Date();
    ...
    if (!projects || projects.length === 0) {
      return NextResponse.json({ sent: 0, checked: 0, message: 'No projects found' });
    }
    ...
    for (const project of projects) {
      ...
      // For testing, we'll send reminders regardless of time (comment out the time check)
      // In production, this would check: if (Math.abs(currentTimeMinutes - reminderTimeMinutes) > 15) continue;
      ...
      if (alertSettings.checkin_reminder_for_foreman !== false && memberships) {
        const foremanIds = memberships.filter((m) => m.role === 'foreman').map((m) => m.user_id);
        usersToRemind.push(...foremanIds);
      }

      // Add admin if enabled
      if (alertSettings.checkin_reminder_for_admin !== false && memberships) {
        const adminIds = memberships.filter((m) => m.role === 'admin').map((m) => m.user_id);
    ...
    console.error('Error in test cron check-in reminders:', error);
    return NextResponse.json(
      {
