-- EPIC 52: Add work orders to planning calendar
-- Update get_planning_data function to include work orders
...
-- Add index for work orders planning queries
CREATE INDEX IF NOT EXISTS idx_work_orders_org_planned_start_status 
    ON work_orders(organization_id, planned_start_at, status) 
    WHERE status != 'cancelled';

CREATE INDEX IF NOT EXISTS idx_work_orders_project_planned_start 
    ON work_orders(project_id, planned_start_at);

-- Update comment
COMMENT ON FUNCTION get_planning_data IS 
'EPIC 26.6 + EPIC 52: Optimized planning data fetch - includes assignments, absences, and work orders';
