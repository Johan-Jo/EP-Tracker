-- Optimize get_dashboard_stats_cached to use immediate refresh when cache is stale
DROP FUNCTION IF EXISTS get_dashboard_stats_cached(uuid, uuid, timestamp with time zone);
DROP FUNCTION IF EXISTS get_dashboard_stats_cached(uuid, uuid, date);
...
BEGIN
  ...
  -- If cache is older than 5 minutes, refresh it immediately (user is viewing dashboard)
